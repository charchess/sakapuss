...
def cmd_load(args):
    ...
    if args.all:
        ...
        if not var_specs:
            print(json.dumps({"error": "Either --vars or --all must be specified"}), file=sys.stderr)
            sys.exit(1)
        result = {}
...
def cmd_check(args):
    ...
    target_yaml_path = find_target_module_yaml(module_code, project_root, skill_path=args.skill_path)
...
def cmd_resolve_defaults(args):
    ...
    try:
        ...
        print(json.dumps({"error": f"Invalid JSON in --core-answers: {e}"}), file=sys.stderr)
        sys.exit(1)
    ...
    if not target_yaml_path:
        print(json.dumps({"error": f"No module.yaml found for module: {module_code}"}), file=sys.stderr)
        sys.exit(1)

    module_def = load_module_yaml(target_yaml_path)
    if not module_def:
        print(json.dumps({"error": f"Failed to parse module.yaml at: {target_yaml_path}"}), file=sys.stderr)
        sys.exit(1)
...
def cmd_write(args):
    ...
    if not project_root:
        if args.project_root:
            ...
            print(json.dumps({"error": "Project root not found and --project-root not provided"}), file=sys.stderr)
            sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(description="BMad Init — Project configuration bootstrap and config loader.")
    ...
    # --- resolve-defaults ---
    resolve_parser = subparsers.add_parser("resolve-defaults", help="Resolve module defaults given core answers")
